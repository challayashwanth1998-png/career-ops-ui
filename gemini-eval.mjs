...
try {
  ...
  if (result.response.candidates[0].finishReason !== 'STOP') {
    console.warn('\n⚠️ Generation stopped early. Reason:', result.response.candidates[0].finishReason);
  }
} catch (err) {
  ...
  if (err.message?.includes('API_KEY')) {
    ...
    console.error('    You have hit the Gemini API rate limit or daily quota. Please try again later.');
  }
  process.exit(1);
}
...
`;
    ...
    const trackerRow = `| ${num} | ${today} | ${company} | ${role} | ${score} | Evaluated | ❌ | [${num}](reports/${filename}) |\n`;
    
    if (!existsSync(trackerPath)) {
      if (!existsSync(join(ROOT, 'data'))) {
        mkdirSync(join(ROOT, 'data'), { recursive: true });
      }
      writeFileSync(trackerPath, '| # | Date | Company | Role | Score | Status | PDF | Report |\n|---|---|---|---|---|---|---|---|\n' + trackerRow);
    } else {
      appendFileSync(trackerPath, trackerRow);
    }
    
    console.log(`\n📊  Added to Application Tracker (data/applications.md):`);
    console.log(`    ${trackerRow.trim()}`);
  } catch (err) {
